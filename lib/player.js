...
Player.prototype.load = function(notes) {
  ...
  notes.forEach(function(note) {
    ...
    queue[start].push({
      drive: note.voice,
      frequency: music.toFrequency(note.note, note.accidental, note.octave)
    });
...
Player.prototype.setQueue = function(queue) {
  this.deltas = Object.keys(queue).map(Number).sort(function(a, b) {
    // This is ridiculous.
    return a - b;
  });
